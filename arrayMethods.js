...
// CODE HERE
const evenNumbers = mixedNumbers.filter(param => param %2 === 0)
...
for (let i = 0; i < myStrongest.length; i++){
  let monster = myStrongest[i];
  console.log(`${monster.monster} CP: ${monster.CP}`)
}
...
let bobsTotal = purchases.filter(elem => elem === 'Bob').reduce((a,c) => a+c.price, 0)
